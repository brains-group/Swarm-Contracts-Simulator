...
#define FOR_EACH_P_AGAIN()     FOR_EACH_P_HELPER
#define FOR_EACH_P(MACRO, ...) __VA_OPT__(EXPAND(FOR_EACH_P_HELPER(MACRO, __VA_ARGS__)))
...
#define FOR_EACH_LIST(MACRO, ...) __VA_OPT__(EXPAND(FOR_EACH_LIST_HELPER(MACRO, __VA_ARGS__)))
...
#define FOR_EACH_LIST_P(MACRO, ...) __VA_OPT__(EXPAND(FOR_EACH_LIST_P_HELPER(MACRO, __VA_ARGS__)))
...
#define CONCEPT_METHOD_CONST(NAME, RETURN, ...)                                          \
    {                                                                                    \
        std::declval<const ConceptType>().NAME(FOR_EACH_LIST(WRAP_DECLVAL, __VA_ARGS__)) \
    } noexcept -> std::same_as<RETURN>
